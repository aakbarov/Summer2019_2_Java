...
public class WarmUp {
	...
	/*
	 * 1. Write a program that can print out the unique values from an int Array Ex:
	 * if arr -> {1,1,2,3,3} output: 2 if arr -> {1,2,2,3,4,4} output: 1 3
	 */
		...
		for (int j = 0; j < arr.length; j++) {
			...
			for (int i = 0; i < arr.length; i++) {
				if (arr[i] == arr[j]) { // compares each index of the array with the given value
					count++; // every time if the value is occurred in the array, count will be increased by
								// one
				}
			}
